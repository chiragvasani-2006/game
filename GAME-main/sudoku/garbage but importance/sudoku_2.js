//all div get by id  for induavatiol selection to perfom task

let B1box1 = document.getElementById("1box1");     let B1box1Text;
let B1box2 = document.getElementById("1box2");     let B1box2Text;
let B1box3 = document.getElementById("1box3");     let B1box3Text;
let B1box4 = document.getElementById("1box4");     let B1box4Text;
let B1box5 = document.getElementById("1box5");     let B1box5Text;
let B1box6 = document.getElementById("1box6");     let B1box6Text;
let B1box7 = document.getElementById("1box7");     let B1box7Text;
let B1box8 = document.getElementById("1box8");     let B1box8Text;
let B1box9 = document.getElementById("1box9");     let B1box9Text;

let B2box1 = document.getElementById("2box1");     let B2box1Text;
let B2box2 = document.getElementById("2box2");     let B2box2Text;
let B2box3 = document.getElementById("2box3");     let B2box3Text;
let B2box4 = document.getElementById("2box4");     let B2box4Text;
let B2box5 = document.getElementById("2box5");     let B2box5Text;
let B2box6 = document.getElementById("2box6");     let B2box6Text;
let B2box7 = document.getElementById("2box7");     let B2box7Text;
let B2box8 = document.getElementById("2box8");     let B2box8Text;
let B2box9 = document.getElementById("2box9");     let B2box9Text;

let B3box1 = document.getElementById("3box1");     let B3box1Text;
let B3box2 = document.getElementById("3box2");     let B3box2Text;
let B3box3 = document.getElementById("3box3");     let B3box3Text;
let B3box4 = document.getElementById("3box4");     let B3box4Text;
let B3box5 = document.getElementById("3box5");     let B3box5Text;
let B3box6 = document.getElementById("3box6");     let B3box6Text;
let B3box7 = document.getElementById("3box7");     let B3box7Text;
let B3box8 = document.getElementById("3box8");     let B3box8Text;
let B3box9 = document.getElementById("3box9");     let B3box9Text;

let B4box1 = document.getElementById("4box1");     let B4box1Text;
let B4box2 = document.getElementById("4box2");     let B4box2Text;
let B4box3 = document.getElementById("4box3");     let B4box3Text;
let B4box4 = document.getElementById("4box4");     let B4box4Text;
let B4box5 = document.getElementById("4box5");     let B4box5Text;
let B4box6 = document.getElementById("4box6");     let B4box6Text;
let B4box7 = document.getElementById("4box7");     let B4box7Text;
let B4box8 = document.getElementById("4box8");     let B4box8Text;
let B4box9 = document.getElementById("4box9");     let B4box9Text;

let B5box1 = document.getElementById("5box1");     let B5box1Text;
let B5box2 = document.getElementById("5box2");     let B5box2Text;
let B5box3 = document.getElementById("5box3");     let B5box3Text;
let B5box4 = document.getElementById("5box4");     let B5box4Text;
let B5box5 = document.getElementById("5box5");     let B5box5Text;
let B5box6 = document.getElementById("5box6");     let B5box6Text;
let B5box7 = document.getElementById("5box7");     let B5box7Text;
let B5box8 = document.getElementById("5box8");     let B5box8Text;
let B5box9 = document.getElementById("5box9");     let B5box9Text;

let B6box1 = document.getElementById("6box1");     let B6box1Text;
let B6box2 = document.getElementById("6box2");     let B6box2Text;
let B6box3 = document.getElementById("6box3");     let B6box3Text;
let B6box4 = document.getElementById("6box4");     let B6box4Text;
let B6box5 = document.getElementById("6box5");     let B6box5Text;
let B6box6 = document.getElementById("6box6");     let B6box6Text;
let B6box7 = document.getElementById("6box7");     let B6box7Text;
let B6box8 = document.getElementById("6box8");     let B6box8Text;
let B6box9 = document.getElementById("6box9");     let B6box9Text;

let B7box1 = document.getElementById("7box1");     let B7box1Text;
let B7box2 = document.getElementById("7box2");     let B7box2Text;
let B7box3 = document.getElementById("7box3");     let B7box3Text;
let B7box4 = document.getElementById("7box4");     let B7box4Text;
let B7box5 = document.getElementById("7box5");     let B7box5Text;
let B7box6 = document.getElementById("7box6");     let B7box6Text;
let B7box7 = document.getElementById("7box7");     let B7box7Text;
let B7box8 = document.getElementById("7box8");     let B7box8Text;
let B7box9 = document.getElementById("7box9");     let B7box9Text;

let B8box1 = document.getElementById("8box1");     let B8box1Text;
let B8box2 = document.getElementById("8box2");     let B8box2Text;
let B8box3 = document.getElementById("8box3");     let B8box3Text;
let B8box4 = document.getElementById("8box4");     let B8box4Text;
let B8box5 = document.getElementById("8box5");     let B8box5Text;
let B8box6 = document.getElementById("8box6");     let B8box6Text;
let B8box7 = document.getElementById("8box7");     let B8box7Text;
let B8box8 = document.getElementById("8box8");     let B8box8Text;
let B8box9 = document.getElementById("8box9");     let B8box9Text;

let B9box1 = document.getElementById("9box1");     let B9box1Text;
let B9box2 = document.getElementById("9box2");     let B9box2Text;
let B9box3 = document.getElementById("9box3");     let B9box3Text;
let B9box4 = document.getElementById("9box4");     let B9box4Text;
let B9box5 = document.getElementById("9box5");     let B9box5Text;
let B9box6 = document.getElementById("9box6");     let B9box6Text;
let B9box7 = document.getElementById("9box7");     let B9box7Text;
let B9box8 = document.getElementById("9box8");     let B9box8Text;
let B9box9 = document.getElementById("9box9");     let B9box9Text;


// select 1 to 9 number by usig btn(button ) Id

let btn1 = document.getElementById("btn1");
let btn2 = document.getElementById("btn2");
let btn3 = document.getElementById("btn3");
let btn4 = document.getElementById("btn4");
let btn5 = document.getElementById("btn5");
let btn6 = document.getElementById("btn6");
let btn7 = document.getElementById("btn7");
let btn8 = document.getElementById("btn8");
let btn9 = document.getElementById("btn9");

//for save 1 to 9 clicked button value

let print = 0;

btn1.addEventListener("click", () => {
    print = "1";
});

btn2.addEventListener("click", () => {
    print = "2";
});

btn3.addEventListener("click", () => {
    print = "3";
});

btn4.addEventListener("click", () => {
    print = "4";
});

btn5.addEventListener("click", () => {
    print = "5";
});

btn6.addEventListener("click", () => {
    print = "6";
});

btn7.addEventListener("click", () => {
    print = "7";
});

btn8.addEventListener("click", () => {
    print = "8";
});

btn9.addEventListener("click", () => {
    print = "9";
});



// now add event listner for print value on div


//for colom one


B1box1.addEventListener("click", () => {
     B1box1.innerText = print;
     B1box1Text = print;
 });

B1box2.addEventListener("click", () => {
    B1box2.innerText = print;
    B1box2Text = print;
});

B1box3.addEventListener("click", () => {
    B1box3.innerText = print;
    B1box3Text = print;
});

B1box4.addEventListener("click", () => {
    B1box4.innerText = print;
    B1box4Text = print;
});

B1box5.addEventListener("click", () => {
    B1box5.innerText = print;
    B1box5Text = print;
});

B1box6.addEventListener("click", () => {
    B1box6.innerText = print;
    B1box6Text = print;
});

B1box7.addEventListener("click", () => {
    B1box7.innerText = print;
    B1box7Text = print;
});

B1box8.addEventListener("click", () => {
    B1box8.innerText = print;
    B1box8Text = print;
});

B1box9.addEventListener("click", () => {
    B1box9.innerText = print;
    B1box9Text = print;
});


//for colom 2

B2box1.addEventListener("click", () => {
    B2box1.innerText = print;
    B2box1Text = print;
});

B2box2.addEventListener("click", () => {
   B2box2.innerText = print;
   B2box2Text = print;
});

B2box3.addEventListener("click", () => {
   B2box3.innerText = print;
   B2box3Text = print;
});

B2box4.addEventListener("click", () => {
   B2box4.innerText = print;
   B2box4Text = print;
});

B2box5.addEventListener("click", () => {
   B2box5.innerText = print;
   B2box5Text = print;
});

B2box6.addEventListener("click", () => {
   B2box6.innerText = print;
   B2box6Text = print;
});

B2box7.addEventListener("click", () => {
   B2box7.innerText = print;
   B2box7Text = print;
});

B2box8.addEventListener("click", () => {
   B2box8.innerText = print;
   B2box8Text = print;
});

B2box9.addEventListener("click", () => {
   B2box9.innerText = print;
   B2box9Text = print;
});

//for colom 3

B3box1.addEventListener("click", () => {
    B3box1.innerText = print;
    B3box1Text = print;
});

B3box2.addEventListener("click", () => {
   B3box2.innerText = print;
   B3box2Text = print;
});

B3box3.addEventListener("click", () => {
   B3box3.innerText = print;
   B3box3Text = print;
});

B3box4.addEventListener("click", () => {
   B3box4.innerText = print;
   B3box4Text = print;
});

B3box5.addEventListener("click", () => {
   B3box5.innerText = print;
   B3box5Text = print;
});

B3box6.addEventListener("click", () => {
   B3box6.innerText = print;
   B3box6Text = print;
});

B3box7.addEventListener("click", () => {
   B3box7.innerText = print;
   B3box7Text = print;
});

B3box8.addEventListener("click", () => {
   B3box8.innerText = print;
   B3box8Text = print;
});

B3box9.addEventListener("click", () => {
   B3box9.innerText = print;
   B3box9Text = print;
});

//for colom 4

B4box1.addEventListener("click", () => {
    B4box1.innerText = print;
    B4box1Text = print;
});

B4box2.addEventListener("click", () => {
   B4box2.innerText = print;
   B4box2Text = print;
});

B4box3.addEventListener("click", () => {
   B4box3.innerText = print;
   B4box3Text = print;
});

B4box4.addEventListener("click", () => {
   B4box4.innerText = print;
   B4box4Text = print;
});

B4box5.addEventListener("click", () => {
   B4box5.innerText = print;
   B4box5Text = print;
});

B4box6.addEventListener("click", () => {
   B4box6.innerText = print;
   B4box6Text = print;
});

B4box7.addEventListener("click", () => {
   B4box7.innerText = print;
   B4box7Text = print;
});

B4box8.addEventListener("click", () => {
   B4box8.innerText = print;
   B4box8Text = print;
});

B4box9.addEventListener("click", () => {
   B4box9.innerText = print;
   B4box9Text = print;
});


//for colom 5


B5box1.addEventListener("click", () => {
    B5box1.innerText = print;
    B5box1Text = print;
});

B5box2.addEventListener("click", () => {
   B5box2.innerText = print;
   B5box2Text = print;
});

B5box3.addEventListener("click", () => {
   B5box3.innerText = print;
   B5box3Text = print;
});

B5box4.addEventListener("click", () => {
   B5box4.innerText = print;
   B5box4Text = print;
});

B5box5.addEventListener("click", () => {
   B5box5.innerText = print;
   B5box5Text = print;
});

B5box6.addEventListener("click", () => {
   B5box6.innerText = print;
   B5box6Text = print;
});

B5box7.addEventListener("click", () => {
   B5box7.innerText = print;
   B5box7Text = print;
});

B5box8.addEventListener("click", () => {
   B5box8.innerText = print;
   B5box8Text = print;
});

B5box9.addEventListener("click", () => {
   B5box9.innerText = print;
   B5box9Text = print;
});


//for colom 6

B6box1.addEventListener("click", () => {
    B6box1.innerText = print;
    B6box1Text = print;
});

B6box2.addEventListener("click", () => {
   B6box2.innerText = print;
   B6box2Text = print;
});

B6box3.addEventListener("click", () => {
   B6box3.innerText = print;
   B6box3Text = print;
});

B6box4.addEventListener("click", () => {
   B6box4.innerText = print;
   B6box4Text = print;
});

B6box5.addEventListener("click", () => {
   B6box5.innerText = print;
   B6box5Text = print;
});

B6box6.addEventListener("click", () => {
   B6box6.innerText = print;
   B6box6Text = print;
});

B6box7.addEventListener("click", () => {
   B6box7.innerText = print;
   B6box7Text = print;
});

B6box8.addEventListener("click", () => {
   B6box8.innerText = print;
   B6box8Text = print;
});

B6box9.addEventListener("click", () => {
   B6box9.innerText = print;
   B6box9Text = print;
});


//for colom 7


B7box1.addEventListener("click", () => {
    B7box1.innerText = print;
    B7box1Text = print;
});

B7box2.addEventListener("click", () => {
   B7box2.innerText = print;
   B7box2Text = print;
});

B7box3.addEventListener("click", () => {
   B7box3.innerText = print;
   B7box3Text = print;
});

B7box4.addEventListener("click", () => {
   B7box4.innerText = print;
   B7box4Text = print;
});

B7box5.addEventListener("click", () => {
   B7box5.innerText = print;
   B7box5Text = print;
});

B7box6.addEventListener("click", () => {
   B7box6.innerText = print;
   B7box6Text = print;
});

B7box7.addEventListener("click", () => {
   B7box7.innerText = print;
   B7box7Text = print;
});

B7box8.addEventListener("click", () => {
   B7box8.innerText = print;
   B7box8Text = print;
});

B7box9.addEventListener("click", () => {
   B7box9.innerText = print;
   B7box9Text = print;
});


//for colom 8


B8box1.addEventListener("click", () => {
    B8box1.innerText = print;
    B8box1Text = print;
});

B8box2.addEventListener("click", () => {
   B8box2.innerText = print;
   B8box2Text = print;
});

B8box3.addEventListener("click", () => {
   B8box3.innerText = print;
   B8box3Text = print;
});

B8box4.addEventListener("click", () => {
   B8box4.innerText = print;
   B8box4Text = print;
});

B8box5.addEventListener("click", () => {
   B8box5.innerText = print;
   B8box5Text = print;
});

B8box6.addEventListener("click", () => {
   B8box6.innerText = print;
   B8box6Text = print;
});

B8box7.addEventListener("click", () => {
   B8box7.innerText = print;
   B8box7Text = print;
});

B8box8.addEventListener("click", () => {
   B8box8.innerText = print;
   B8box8Text = print;
});

B8box9.addEventListener("click", () => {
   B8box9.innerText = print;
   B8box9Text = print;
});


//for colom 9


B9box1.addEventListener("click", () => {
    B9box1.innerText = print;
    B9box1Text = print;
});

B9box2.addEventListener("click", () => {
   B9box2.innerText = print;
   B9box2Text = print;
});

B9box3.addEventListener("click", () => {
   B9box3.innerText = print;
   B9box3Text = print;
});

B9box4.addEventListener("click", () => {
   B9box4.innerText = print;
   B9box4Text = print;
});

B9box5.addEventListener("click", () => {
   B9box5.innerText = print;
   B9box5Text = print;
});

B9box6.addEventListener("click", () => {
   B9box6.innerText = print;
   B9box6Text = print;
});

B9box7.addEventListener("click", () => {
   B9box7.innerText = print;
   B9box7Text = print;
});

B9box8.addEventListener("click", () => {
   B9box8.innerText = print;
   B9box8Text = print;
});

B9box9.addEventListener("click", () => {
   B9box9.innerText = print;
   B9box9Text = print;
});
//just for test

//B1box1.innerText = print;