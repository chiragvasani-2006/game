...
// Function to initialize the game (fill the grid with random valid numbers)
function generateSudoku() {
    // Random Sudoku generator logic (simplified for the sake of demonstration)
    let board = [
        [5, 3, 0, 0, 7, 0, 0, 0, 0],
        [6, 0, 0, 1, 9, 5, 0, 0, 0],
        [0, 9, 8, 0, 0, 0, 0, 6, 0],
...
boxes.forEach((box) => {
    box.addEventListener("click", () => {
        ...
        if (box.innerText === "" && selectedNumber !== 0 && isValidMove(row, col, selectedNumber)) {
            box.innerText = selectedNumber;
        }
    });
});

// Check if the number can be placed in the selected box
...
document.getElementById("newGameBtn").addEventListener("click", generateSudoku);

// Button to check if the game is completed correctly
document.getElementById("checkGameBtn").addEventListener("click", checkGameCompleted);

// Initialize the game with a Sudoku grid
generateSudoku();
